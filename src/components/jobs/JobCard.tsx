    ...
    <Link href={`/jobs/${job.slug}`} className="block group">
      <article className={`bg-white rounded-xl border transition-all duration-200 p-5 hover:shadow-md hover:-translate-y-0.5 ${job.featured ? 'border-brand-200 shadow-sm' : 'border-slate-200'}`}>
        ...
        <div className="flex flex-wrap items-center gap-2 mb-3">
          ...
          <span className={`inline-flex items-center text-xs font-medium px-2.5 py-1 rounded-full ${expStyle[job.experience] ?? 'bg-slate-100 text-slate-600'}`}>
            {job.experience}
          </span>
        ...
        <div className="flex items-center justify-between gap-2 flex-wrap">
          <div className="flex flex-wrap gap-1.5">
            {job.tags.slice(0, 3).map((tag) => (
              <span key={tag} className="text-xs text-slate-500 bg-slate-50 border border-slate-200 px-2 py-0.5 rounded">
                {tag}
              </span>
            ))}
          </div>
          <div className="text-right flex-shrink-0">
            <div className="text-sm font-semibold text-slate-900">{formatSalary(job.salaryMin, job.salaryMax)}</div>
            <div className="text-xs text-slate-400">{timeAgo(job.postedAt)}</div>
          </div>
        </div>
      </article>
    </Link>
