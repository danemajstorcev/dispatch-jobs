          ...
          <div className="space-y-4">
            <div className="bg-white border border-slate-200 rounded-xl p-6 sm:p-8 shadow-sm">
              <div className="flex items-start gap-4 mb-5">
                ...
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    ...
                    {job.featured && (
                      <span className="text-xs font-semibold bg-brand-50 text-brand-700 border border-brand-200 px-2 py-0.5 rounded-full">
                        Featured
                      </span>
                    )}
                  </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
                {[
                  ...
                  { icon: "⭐", label: "Experience", value: job.experience },
                  {
                    icon: "💰",
                    label: "Salary",
                    value: formatSalary(job.salaryMin, job.salaryMax) + "/yr",
                  },
              ...
              <div className="flex flex-wrap items-center gap-2">
                <span
                  className={`text-sm font-medium px-3 py-1 rounded-full border ${workModeColor[job.workMode] ?? "bg-slate-100 text-slate-600 border-slate-200"}`}
                >
                  {job.workMode}
                </span>
                {job.tags.map((tag) => (
                  <span
                    key={tag}
                    className="text-xs text-slate-500 bg-slate-100 border border-slate-200 px-2.5 py-1 rounded-full"
                  >
                    {tag}
                  </span>
                ))}
              ...
              <ul className="space-y-2.5">
                {job.requirements.map((r, i) => (
                  <li
                    ...
                    className="flex items-start gap-2.5 text-sm text-slate-600"
                  >
                    <span className="text-brand-500 font-bold flex-shrink-0 mt-0.5">
                      ✓
                    </span>
                    {r}
                  </li>
